...
impl ResponseError for AuthError {
    fn error_response(&self) -> HttpResponse {
        match self { 
            ...
            AuthError::DuplicateValue(ref message) => HttpResponse::BadRequest().json(message),            AuthError::GenericError(ref message) => HttpResponse::BadRequest().json(message),
        }
    }
}
impl From<UuidError> for AuthError {
    ...
    fn from(error: DBError) -> AuthError {
        match error {
            ...
            _ => AuthError::GenericError(String::from("A database error occured (this isn't very helpful)")),
        }
    }
}